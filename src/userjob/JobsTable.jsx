      ...
      <Table className="rounded-xl border border-richblack-800">
        <Thead>
          <Tr className="flex gap-x-[90px] rounded-t-md border-b border-b-richblack-800 px-6 py-2">
            <Th className="flex-1 text-left text-sm font-medium uppercase text-richblack-100">Art-Title</Th>
        ...
        <Tbody>
          {jobs?.length === 0 ? (
            <Tr>
              <Td className="py-10 text-center text-2xl font-medium text-richblack-100">No arts posted</Td>
            </Tr>
          ) : (
            jobs?.map((job) => (
              <Tr key={job._id  } className="flex gap-x-20 border-b border-richblack-800 px-6 py-8">
                <Td className="flex flex-1 gap-x-14">
                  <div>
                  <h3 className='mb-2 w-[220px]'> {job.companyName}</h3>
                  <p className='text-gray-500 line-clamp-3'>{job.companyDescription}</p>
                  </div>
